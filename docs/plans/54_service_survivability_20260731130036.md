...
- **2026-07-31 — code-reviewer pass 2 (adversarial)**: FAIL (0 CRITICAL / 1 WARNING / 1 INFO).
  ...
  - **I-2 (ESCALATED to user — physical constraint of the ratified design, not a code defect)** — the US2 acceptance criterion "An explicit stop is never resurrected" is worded absolutely, but the `false` flag write is a **bounded** (2s) synchronous main-thread write per the user-ratified ANR-safe, race-free design (P54-005/P54-014). A pathological >2s DataStore stall on the exact STOP could abandon the `false` write, leaving `server_running=true`, and a later `MY_PACKAGE_REPLACED` (an app update, typically much later) could then restart a stopped server. This cannot be made an absolute guarantee without either (a) an unbounded main-thread write (violates ANR-safety — a hard project rule), or (b) an async fallback write (reintroduces the P54-005 last-writer race). A bounded retry does not guarantee commit and only lengthens the main-thread block (weasel, rejected). The acceptance-criterion wording cannot be edited (sacred doc). Disposition: **left for the user's final call** — accept the astronomically-low-probability edge (device in >2s I/O starvation at the instant of stop, with no intervening successful write before an update), or change one of the ratified tradeoffs.
