...
android {
    ...
    // Release applicationId is identical for both flavors; debug builds get a per-flavor suffix
    // so gms/foss debug builds can be installed side-by-side.
...
androidComponents {
    // Per-flavor debug applicationId (`…mcp.gms.debug` / `…mcp.foss.debug`) so both
    // debug builds coexist while release keeps the branded application ID.
    onVariants(selector().withBuildType("debug")) { variant ->
        variant.applicationId.set(
            "com.yedhant.androidremotecontrolmcp.${variant.flavorName}.debug",
        )
    }
    onVariants { variant ->
