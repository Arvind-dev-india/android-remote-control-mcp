...
class ExportedComponentsManifestTest {
    ...
    @Test
    fun `adb-facing components are confined to the debug manifest and require DUMP`() {
        val mainNames = componentsIn(MAIN_MANIFEST).map { it.name }.toSet()
        val debugByName = componentsIn(DEBUG_MANIFEST).associateBy { it.name }
        listOf(
            ".services.mcp.AdbConfigReceiver",
            ".services.mcp.AdbServiceTrampolineActivity",
        ).forEach { name ->
            assertTrue(name !in mainNames, "$name must not be declared in $MAIN_MANIFEST")
            val component = debugByName[name] ?: error("$name is not declared in $DEBUG_MANIFEST")
